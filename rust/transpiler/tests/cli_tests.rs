...
fn stdin_invalid_source_exits_nonzero() {
    ...
    child
        ...
        .take()
        .unwrap()
        .write_all(b"<div>unclosed")
        .unwrap();
    let output = child.wait_with_output().unwrap();

    assert!(
        !output.status.success(),
        "Should exit non-zero for invalid source"
    );
...
fn json_output_has_compiled_and_mappings() {
    ...
    assert!(
        compiled.contains("def Render"),
        "Compiled code should contain function"
    );
}
...
fn daemon_ready_and_compile() {
    ...
    stdout
        ...
        .expect("Should read response payload");
    ...
    assert!(
        json.get("compiled").is_some(),
        "Response should have 'compiled' field"
    );
    let compiled = json["compiled"].as_str().unwrap();
    assert!(
        compiled.contains("def Render"),
