...
from hyper_content._mixins import CollectionMixin, SingletonMixin
...
class _MarkdownCollectionDescriptor:
    ...
    def __instancecheck__(self, instance):
        ...
        return isinstance(instance, CollectionMixin) and isinstance(instance, _MarkdownMixin)

    def __subclasscheck__(self, subclass):
        """Support issubclass(cls, MarkdownCollection)."""
        return issubclass(subclass, CollectionMixin) and issubclass(subclass, _MarkdownMixin)
    ...
    def __mro_entries__(self, bases):
        ...
        if HAS_MSGSPEC:
            ...
            for i, b in enumerate(bases):
                if b is msgspec.Struct or (isinstance(b, type) and issubclass(b, msgspec.Struct)):
                    struct_idx = i
                    break
            ...
            if struct_idx is not None:
                ...
                if struct_idx == 0:
                    raise TypeError(
                        f"MarkdownCollection must come before msgspec.Struct in the base class list.\n"
                        f"Use: class YourClass(MarkdownCollection, msgspec.Struct)\n"
                        f"Not: class YourClass(msgspec.Struct, MarkdownCollection)"
                    )
...
class _MarkdownSingletonDescriptor:
    ...
    def __instancecheck__(self, instance):
        ...
        return isinstance(instance, SingletonMixin) and isinstance(instance, _MarkdownMixin)

    def __subclasscheck__(self, subclass):
        """Support issubclass(cls, MarkdownSingleton)."""
        return issubclass(subclass, SingletonMixin) and issubclass(subclass, _MarkdownMixin)
    ...
    def __mro_entries__(self, bases):
        ...
        if HAS_MSGSPEC:
            ...
            if struct_idx is not None:
                ...
                if struct_idx == 0:
                    raise TypeError(
                        f"MarkdownSingleton must come before msgspec.Struct in the base class list.\n"
                        f"Use: class YourClass(MarkdownSingleton, msgspec.Struct)\n"
                        f"Not: class YourClass(msgspec.Struct, MarkdownSingleton)"
                    )
