...
from hyper.content import Singleton, Collection, load
...
def test_before_parse_hook_collection(content_dir):
    ...
    posts_dir.mkdir()
    (posts_dir / "post1.json").write_text('{"title": "DRAFT: Hello", "status": "draft"}')
    (posts_dir / "post2.json").write_text('{"title": "DRAFT: World", "status": "draft"}')
...
def test_after_parse_hook_singleton(content_dir):
    ...
    (content_dir / "article.json").write_text('{"title": "Hello World", "body": "This is a test article"}')
...
def test_after_parse_hook_collection(content_dir):
    ...
    posts_dir = content_dir / "posts"
    posts_dir.mkdir()
    (posts_dir / "post1.json").write_text('{"title": "First", "content": "Hello world"}')
    (posts_dir / "post2.json").write_text('{"title": "Second", "content": "Foo bar baz"}')
...
def test_after_parse_hook_field_transformation(content_dir):
    ...
    (content_dir / "legacy.json").write_text('{"old_name": "value", "deprecated_field": 123}')
...
def test_after_parse_hook_filter_sensitive_data(content_dir):
    ...
    (content_dir / "user.json").write_text('{"name": "Alice", "email": "alice@example.com", "password": "secret123"}')
...
def test_before_parse_hook_with_path_context(content_dir):
    ...
    (published_dir / "post2.json").write_text('{"title": "Published Post", "status": "published"}')
