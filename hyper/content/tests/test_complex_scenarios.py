...
def test_pydantic_field_validators(content_dir):
    ...
    # Invalid email
    (content_dir / "invalid-email.json").write_text('{"email": "notanemail", "age": 30}')
    with pytest.raises(ValueError, match="Invalid email"):
        load("invalid-email.json", User)

    # Invalid age
    (content_dir / "invalid-age.json").write_text('{"email": "test@example.com", "age": 200}')
...
def test_msgspec_post_init_validation(content_dir):
    ...
    # Valid data
    (content_dir / "config.json").write_text('{"api_key": "secret1234567", "rate_limit": 100}')
    config = Config.load()
    assert config.api_key == "secret1234567"
    assert config.rate_limit == 100

    # Invalid api_key (too short) - msgspec wraps the error
    (content_dir / "invalid-key.json").write_text('{"api_key": "short", "rate_limit": 100}')
    ...
    (content_dir / "invalid-rate.json").write_text('{"api_key": "secret1234567", "rate_limit": -1}')
    with pytest.raises((ValueError, msgspec.ValidationError)):
...
def test_computed_properties(content_dir):
    ...
    products_dir.mkdir()
    (products_dir / "item1.json").write_text('{"name": "Laptop", "price": 1000.0, "tax_rate": 0.15}')
...
def test_merge_strategy_deep(content_dir):
    ...
    assert config.features == {"auth": True, "api": False, "admin": True}  # All keys present!
