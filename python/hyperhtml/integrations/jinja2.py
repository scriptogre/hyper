...
class HyperExtension(Extension):
    ...
    def _parse_hyper(self, parser, token):
        ...
        matching keyword argument. The slot HTML reaches the component at the
        call site, exactly as the Django tag passes it. No
        ...
        # as `name=[var]` (list-wrapped, since the component uses `yield from`).
        setups = []
        default_body = []
        for node in body:
            ...
            setups.append(
                nodes.AssignBlock(nodes.Name(var.name, "store"), None, node.body)
            )
            call.kwargs.append(
                nodes.Keyword(node.name, nodes.List([nodes.Name(var.name, "load")]))
            )
        ...
        if has_default:
            ...
            setups.append(
                nodes.AssignBlock(nodes.Name(var.name, "store"), None, default_body)
            )
            call.kwargs.append(
                nodes.Keyword("content", nodes.List([nodes.Name(var.name, "load")]))
            )
    ...
    def _parse_slot(self, parser, token):
        ...
        if not getattr(parser, "_hyper_depth", 0):
            parser.fail(
                "{% slot %} must appear inside a {% hyper %} block", token.lineno
            )
